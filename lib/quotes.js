...
export function catalogPriceForPack(packId, billing) {
  return packUnitPrice(packId, billing) || null;
}

export function formatLineDiscountLabel(line, billing) {
  if (isIaFirstMonthLine(line)) {
    if (billing === "ANNUAL") {
...
export function hasIaFirstMonthPromo(lines) {
  return lines.some((l) => isIaFirstMonthLine(l));
}

export function countIaContractMonths(lines) {
  return lines.reduce((sum, line) => {
    if (line.type !== "PACK" || !line.packId) return sum;
    return sum + (line.quantity || 1);
  }, 0);
}

export const VAT_RATE = 0.21;

export function computeLineTotal(line, _billing = "MONTHLY") {
...
export const QUOTE_BILLING_LABEL = {
  MONTHLY: "Facturación mensual (tarifa de catálogo por mes)",
  ANNUAL: "Facturación anual (−15% en tarifa mensual por mes)",
};
