...
/** Leads, pipeline y presupuestos (rol comercial o equipo operativo). */
...
/** Módulo de finanzas (CEO o rol finanzas). */
export function canAccessFinance(session) {
  return isAdmin(session) || isFinance(session);
}

/** RRHH — solo CEO por ahora. */
export function canAccessHr(session) {
  return isAdmin(session);
}

/** Layout dashboard: comercial, finanzas o RRHH. */
export function canAccessDashboardShell(session) {
  return (
    canAccessSalesCrm(session) ||
    canAccessFinance(session) ||
    canAccessHr(session)
  );
}
