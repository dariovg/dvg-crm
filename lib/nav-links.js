...
const FINANCE_LINK = { href: "/ceo/finanzas", label: "Finanzas", icon: "finance" };
const HR_LINK = { href: "/ceo/rrhh", label: "RRHH", icon: "users" };
const ADMIN_LINKS = [
  { href: "/admin/users", label: "Equipo", icon: "users" },
  { href: "/admin/crm-settings", label: "Ajustes comercial", icon: "settings" },
...
export function getNavSectionsForSession(session) {
  ...
  if (canAccessCeoDashboard(session)) {
    ceoLinks.push(CEO_LINK);
  }
  if (canAccessFinance(session)) {
    ceoLinks.push(FINANCE_LINK);
  }
  if (canAccessHr(session)) {
    ceoLinks.push(HR_LINK);
  }
  if (ceoLinks.length) {
    sections.push({
      id: "ceo",
