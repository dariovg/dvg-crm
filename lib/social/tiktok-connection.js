...
export async function exchangeTikTokCode(code, codeVerifier) {
  ...
  if (env("TIKTOK_USE_PKCE") === "1" && codeVerifier) {
...
export function buildTikTokAuthorizeUrl({ state, codeChallenge } = {}) {
  ...
  // PKCE solo si está explícitamente activado (Login Kit web no lo usa por defecto).
  if (env("TIKTOK_USE_PKCE") === "1" && codeChallenge) {
