...
function formatTwitterError(err) {
  ...
  if (
    /credits|CreditsDepleted|402/i.test(String(detail)) ||
    code === 402
  ) {
    return `${detail} — Tu cuenta de desarrollador X no tiene créditos. Entra en https://console.x.com → Billing → Credits, añade saldo (desde ~5 USD) y vuelve a publicar. No reintentes hasta recargar.`;
  }
