...
export function isPermanentPublishError(message) {
  const m = String(message || "");
  return /credits|CreditsDepleted|402|no tiene créditos/i.test(m);
}
...
export async function recordPublishFailure(post, err) {
  ...
  const permanent = attempts >= MAX_PUBLISH_ATTEMPTS || isPermanentPublishError(message);
