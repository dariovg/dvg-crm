  ...
  providers: [
    CredentialsProvider({
      ...
      async authorize(credentials, req) {
        ...
        return verifyLoginCredentials({ email, password, ipHash, req });
      },
    }),
  ],
  pages: {
  ...
  callbacks: {
    async jwt({ token, user, trigger }) {
      if (user) {
        ...
        token.profileStatus = user.profileStatus ?? "AVAILABLE";
        token.statusMessage = user.statusMessage ?? null;
        token.invalid = false;
        return token;
      }
      ...
      const now = Date.now();
      const lastCheck = token.sessionCheckedAt ?? 0;
      const shouldCheck = now - lastCheck > 5 * 60 * 1000;
      ...
      const lastTouch = token.sessionTouchedAt ?? 0;
      if (token.sessionId && now - lastTouch > 2 * 60 * 1000) {
        await touchUserSession(token.sessionId);
        token.sessionTouchedAt = now;
      }
    ...
    async session({ session, token }) {
      ...
      if (session.user) {
        ...
        session.user.profileStatus = token.profileStatus ?? "AVAILABLE";
        session.user.statusMessage = token.statusMessage ?? null;
