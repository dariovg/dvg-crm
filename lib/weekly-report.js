...
export function formatWeeklyReportEmail(kpis) {
  ...
  const subject = `[${APP_DISPLAY_NAME}] Informe semanal — ${newLeadsLabel(kpis.newLeads)} leads nuevos`;
  const text = [
    `Informe semanal ${APP_DISPLAY_NAME} (${from} – ${to})`,
    "",
    "── KPIs (últimos 7 días) ──",
  ...
  const html = `
    <h2>Informe semanal ${APP_DISPLAY_NAME}</h2>
    <p><em>${from} – ${to}</em></p>
