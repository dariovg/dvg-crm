...
export const QUOTE_TEMPLATES = {
  ...
  IA: {
    ...
    defaultBilling: "MONTHLY",
    notes:
      "Incluye auditoría inicial, configuración de canales y mejoras continuas según plan contratado. Ajusta meses normales en las líneas.",
    customLines: [],
  },
  CONSULTING: {
    id: "CONSULTING",
...
function hasIaPackLines(packId, lines = []) {
  return lines.some((l) => l.type === "PACK" && l.packId === packId);
}
...
export function buildLinesFromTemplate(templateId, billing, catalogPriceForPack, packLineDescription) {
  ...
  if (packId && templateId === "IA") {
    lines.push(
      ...buildDefaultIaPackLines(packId, resolvedBilling).map((l, i) => ({
        ...l,
        sortOrder: i,
      }))
    );
    ...
    lines.push({
      type: "PACK",
      packId,
      description: packLineDescription(packId, resolvedBilling),
      quantity: 1,
...
export function isTemplateInLines(templateId, lines = []) {
  ...
  const hasPack = tpl.defaultPackId
    ? templateId === "IA"
      ? hasIaPackLines(tpl.defaultPackId, lines)
...
export function appendTemplateLines(
  ...
  if (tpl.defaultPackId) {
    const exists =
      templateId === "IA"
        ? hasIaPackLines(tpl.defaultPackId, existingLines)
        : existingLines.some(
    ...
    if (!exists) {
      if (templateId === "IA") {
        toAdd.push(
          ...buildDefaultIaPackLines(tpl.defaultPackId, billing).map((l, i) => ({
            ...l,
        ...
        toAdd.push({
          ...
          description: packLineDescription(tpl.defaultPackId, billing),
          quantity: 1,
