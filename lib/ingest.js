...
export async function cancelBookingRecord({ email, phone, date, time }) {
  const mail = normalizeEmail(email);
  ...
  const phoneClean = cleanPhone(phone);
  if (phoneClean && contact.phone && contact.phone !== phoneClean) {
    return { ok: false, error: "phone_mismatch" };
  }

  const meeting = await prisma.meeting.findFirst({
    where: { contactId: contact.id, date, time },
    orderBy: { createdAt: "desc" },
  });
  ...
  await prisma.contactEvent.create({
    data: {
      ...
      summary: `Cita cancelada desde web: ${date} ${time}`,
      payload: { date, time, source: "web_chat" },
    },
  });
...
export async function updateBookingRecord({
  ...
  if (previousDate && previousTime) {
    meeting = await prisma.meeting.findFirst({
      where: { contactId: contact.id, date: previousDate, time: previousTime },
      orderBy: { createdAt: "desc" },
    });
  }
  if (!meeting) {
  ...
  if (meeting) {
    await prisma.meeting.update({
      ...
      data: {
        ...
        notes: notes != null ? notes : meeting.notes,
      },
    });
  } else {
    await prisma.meeting.create({
  ...
  await prisma.contactEvent.create({
    data: {
      ...
      summary: `Cita actualizada desde web: ${date} ${time}`,
      payload: { date, time, meetUrl, notes, previousDate, previousTime },
    },
  });
