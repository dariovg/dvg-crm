...
export async function notifyAssignment({
  ...
  const subject = `Nuevo ${kind} asignado — ${APP_DISPLAY_NAME}`;
  const text = `Hola ${name},\n\nSe te ha asignado ${kind === "tarea" ? "la tarea" : "el lead"}: ${title}\n\nVer en la app: ${link}`;
  const html = `<p>Hola ${name},</p><p>Se te ha asignado ${kind === "tarea" ? "la tarea" : "el lead"}: <strong>${title}</strong></p><p><a href="${link}">Abrir en la app</a></p>`;
